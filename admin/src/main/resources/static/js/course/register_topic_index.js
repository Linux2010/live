...
jQuery(function ($) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        subGrid: false,
        datatype: "json",
        postData:{isRegisterTopic:0},//is_register_topic 是否是注册考题试卷(0:是注册考题;1:课程考题)
        height: 'auto',
    ...
    jQuery(grid_selector).jqGrid('navGrid', pager_selector,
        {
            ...
            delfunc : del,
            search: false,
            // searchicon : 'ace-icon fa fa-pencil-square-o orange',
            // searchfunc : resetpay,
            refresh: true,
    ...
    //添加考卷考题
    function add(){
        var examJson =  findRegisterExam();
        if(examJson  &&   examJson.course_topic_examination_id){
            Modal.alert({msg: "注册考题已经存在,请选择修改按钮操作", title: "提示", btnok: "确定", btncl: "取消"});
            return false;
        }
    ...
    function edit(id){
        ...
        initCourseTypeCombotree("#edit-form-topic");
        removeTopics("#edit-form-topic");
        topicIndex=0;
        show("#add-form-topic");
        initEditData(rowData);

    }
    function Travel(nodeId){
        var tree=  $("#edit-form-topic").find('#courseTypeId').combotree('tree');
        var roots= tree.tree('getRoots'),children,i,j;
        for(i=0;i<roots.length;i++){
            if(nodeId == roots[i].id){
                //tree.tree('check',roots[i].target);
                $("#edit-form-topic").find('#courseTypeId').combotree("setValue",nodeId);
                break;
            ...
            children=tree.tree('getChildren',roots[i]);
            if(children.length > 0){

                for(j=0;j<children.length;j++){
                    if(nodeId == children[j].id){
    ...
    function initEditData(rowData){
        ...
        if((isRegisterTopic == 0) || !courseId){
            ...
            if(courseTypeId)Travel(courseTypeId);//课程类别选中默认
    ...
    $("#add-form-topic #addTopic").click(function () {
        if(topicIndex >=5){
            Modal.alert({msg: "注册考题最多添加5道！", title: "提示", btnok: "确定", btncl: "取消"});
        }
        ++topicIndex;
        //记录考题题号
   ...
   $("#add-form-topic input[name=isRegisterTopic]").change(function () {
    ...
    //初始化课程下拉框
    function initCourseListByTypeId(courseTypeId){
        $("#courseList").empty();
        $("#courseList").append( '<option value="">--课程名称--</option>');
        $.ajax({
            async: false,
            type: "POST",
            ...
            success: function (data) {
                if(data){
                    $.each(data, function(i,val){
                        $("#courseList").append('<option value="'+val.course_id+'">'+val.course_title+'</option>');
                    });
                }
            },
            error: function (data) {
    ...
    function initCourseTypeCombotree(idTag){
        $(idTag).find('#courseTypeId').combotree ({
            ...
            onClick:function(node){
                // 查询该节点分类下的课程列表
                initCourseListByTypeId(node.id);
                var isRegisterTopic = $("#add-form-topic").find('input[name="isRegisterTopic"]:checked ').val();
                var courseTypeId = $("#add-form-topic").find("input[name=courseTypeId]").val();
...
//查找注册考卷
function findRegisterExam(){
    var examJson;
    $.ajax({
        async:false,
        url:base+"topic/selectRegisterCourseTopicExamination",
        dataType:"json",
        data:{},
        success:function(data){
            examJson=data;
