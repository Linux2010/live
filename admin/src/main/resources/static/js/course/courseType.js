...
jQuery(function($) {
    jQuery(grid_selector).jqGrid({
        ...
        subGrid : false,
        datatype: "json",
        postData:{typeName:"",typeVal:"spkc",parentId:-1},
        height: 'auto',
        loadtext: '正在加载...',
    ...
    function linkChild(cellvalue, options, rowObject){
        var urlVal = base + "courseType/courseTypeChildren?parentId="+rowObject.courseTypeId;
    ...
    function verifyValue(typeName,status,picture){
        ...
        if(isEmpty(picture)){
            Modal.alert({msg: "不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        result = true;
        return result;
...
function searchCtList(){
    var typeVal = $("#typeVal").val();
    $("#grid-table").jqGrid('setGridParam',{
        postData:{typeName:"",typeVal:typeVal}
    }).trigger('reloadGrid');
}
