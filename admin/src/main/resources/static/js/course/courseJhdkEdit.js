$(function(){
        ...
        success: function (courseData) {
            if(courseData != null){
                ...
                $('#courseTypeId').combotree ({
                    url: base + 'courseType/searchCtByParentId?parentId=-1&typeVal=jhdk',
                ...
                $("#courseEndTime").val(new Date(courseData.courseEndTime).pattern("yyyy-MM-dd HH:mm"));
                $("#costPrice").val(courseData.costPrice);
                $("#vipPrice").val(courseData.vipPrice);
                $("#seqno").val(courseData.seqno);
                $("#courseAddress").val(courseData.courseAddress);
                $("#courseAddressDetail").val(courseData.courseAddressDetail);
    ...
    $("#backBtn").click(function(){
        location.href=base+"courseJhdk/courseJhdkIndex";
    });
    ...
    $("#updateCourse").click(function(){
        ...
        if(!verifyValue($("#course_type_id").val(),$("#courseTagId").val(),$("#courseTitle").val(),
                $("#courseBeginTime").val(),$("#courseEndTime").val(),$("#teacherId").val(),
                $("#costPrice").val(),$("#vipPrice").val(),$("#seqno").val(),
                $("#courseIntro").val(),$("#courseAddress").val(),$("#courseAddressDetail").val())){
            return false;
        }else{
            // 设置发布按钮不可用
    ...
    function verifyValue(courseTypeId,courseTagId,courseTitle,
                         courseBeginTime,courseEndTime,teacherId,
                         costPrice,vipPrice,seqno,courseIntro,courseAddress,courseAddressDetail){
        var result = false;
        ...
        if(isEmpty(courseBeginTime)){
            Modal.alert({msg: "开始时间不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        if(isEmpty(courseEndTime)){
            Modal.alert({msg: "结束时间不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        ...
        if(isEmpty(courseAddress)){
            Modal.alert({msg: "地址不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        if(isEmpty(courseAddressDetail)){
            Modal.alert({msg: "详细地址不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        result = true;
        return result;
