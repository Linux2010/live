...
jQuery(function($) {
    jQuery(grid_selector).jqGrid({
        url : base + "courseZb/searchCList",
        subGrid : false,
        datatype: "json",
        height: 'auto',
        loadtext: '正在加载...',
        colNames:['id','序号','直播类型','课程封面','课程名称','开始时间','课程价格','会员价格','讲师名称','课程分类','操作'],
        colModel:[
            {name: 'courseId', index: 'courseId', width: 10, editable: true, hidden:true, key: true},
            {name:'seqno',index:'seqno',width:20},
            {name:'zbType',index:'zbType',width:20,formatter:showZbType},
            ...
            {name:'id',index:'id',width:25,formatter:operFuc}
        ],
        jsonReader : {
            root:"list",
            page: "pageNum",
    ...
    // 展示直播类型
    function showZbType(cellvalue, options, rowObject){
        if(cellvalue == "sp"){
            return "视频直播";
        }else if(cellvalue == "yp"){
            return "音频直播";
        }else{
            return "";
        }
    ...
    function showCourseTopic(cellvalue, options, rowObject){
        ...
        $.ajax({
            async: false,
            type: "POST",
            data: {courseId:rowObject.courseId},
            url: base + "course/searchCtCountByCId",
            dataType: "json",
    ...
    $("#course-search").bind("click",function(){
        ...
        $("#grid-table").jqGrid('setGridParam',{
            postData:{courseTitle:courseTitle,zbType:zbType}
        }).trigger('reloadGrid');
    });

    // 进入课程发布页面
    $("#course-add").click(function(){
        location.href=base+"courseZb/courseZbAdd";
    });
...
function editC(courseId){
    location.href=base+"courseZb/courseZbEdit?courseId="+courseId;
}
...
// 上传课程内容
function uploadC(id){
    Modal.alert({msg: "上传课程内容...", title: "提示", btnok: "确定"});
}

// 上传课程考题
function uploadCt(courseId,courseTitle){
    location.href=base+"topic/course/add?courseId="+courseId+"&courseName="+encodeURIComponent(courseTitle)+"&cType=zbkc";
}

// 查看课程考题
function showCtDetail(courseId){
    location.href=base+"courseZb/showCtDetail?courseId="+courseId;
}