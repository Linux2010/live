$(function(){
    ...
    $('#courseTypeId').combotree ({
        url: base + 'courseType/searchCtByParentId?parentId=-1&typeVal=zbkc',
    ...
    $("#addCourse").click(function(){
        // 先验证非空项
        if(!verifyValue($("#course_type_id").val(),$("#courseTagId").val(),$("#courseCover").val()
                ,$("#courseTitle").val(),$("#courseBeginTime").val(),$("#teacherId").val(),
                $("#costPrice").val(),$("#vipPrice").val(),$("#seqno").val(),
                $("#courseIntro").val(),$("#zbType").val(),$("#courseImg").val())){
            return false;
        }else{
            // 设置发布按钮不可用
    ...
    // 校验对应数据
    function verifyValue(courseTypeId,courseTagId,courseCover,
                         courseTitle,courseBeginTime,teacherId,
                         costPrice,vipPrice,seqno,courseIntro,zbType,courseImg){
        var result = false;
        ...
        if(isEmpty(zbType)){
            Modal.alert({msg: "直播类型不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
