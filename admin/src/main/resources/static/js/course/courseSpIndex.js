...
jQuery(function($) {
    ...
    $("#course-add").click(function(){
        location.href=base+"courseSp/courseSpAdd";
    });
    ...
    $("#uploadCourseSp").click(function(){
        // 先验证非空项
        if(!verifyValue($("#courseId").val(),$("#videoId").val(),$("#objName").val())){
            return false;
        }else{
            // 设置上传按钮不可用
            $("#uploadCourseSp").attr("disabled",true);
            $("#uploadCourseSp").text("上传中...");
            // 如果验证通过，则提交表单
            $("#upload-form-courseSp").ajaxSubmit({
                type:"post",
                dataType:"json",
                url:base+"course/modifyV",
                success: function(data) {
                    // 设置上传按钮可用
                    $("#uploadCourseSp").attr("disabled",false);
                    $("#uploadCourseSp").text("上 传");
                    if(data == 0){
...
function editC(courseId){
    location.href=base+"courseSp/courseSpEdit?courseId="+courseId;
}
...
function uploadCt(courseId,courseTitle){
    ...
    if(examJson && examJson.courseTopicExaminationId ){
        ...
        location.href=base+"topic/course/add?courseId="+courseId+"&courseName="+encodeURIComponent(courseTitle)+"&cType=spkc";
    }
}

// 查看课程考题
function showCtDetail(courseId){
    location.href=base+"courseSp/showCtDetail?courseId="+courseId;
}
