jQuery(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        url : base + "profitshare/relation/getRelationPage?userId=" + overallUserId,
        subGrid : true,
        subGridOptions : {
        ...
        subGridRowExpanded: function(subgrid_id, row_id) { //Grid内部嵌套Grid
            location.href = base + "profitshare/relation/pageskip?relationId=" + row_id + "&level=3";
        }
    });
...
var add = function(){
    location.href = base + "profitshare/relation/secondAdd?relationId="+overallRelationId;
}
...
var editParentRaltion = function (relationId) {
   // if(isD(relationId)){
        location.href = base + "profitshare/relation/editParentRaltion?relationId="+relationId+"&level=2";
  //  }
}
