jQuery(function ($) {
    ...
    $("#addCoupon").click(function(){
        ...
        var couponBeginTimeStr = $form.find("input[name=couponBeginTimeStr]").val();
        var couponEndTimeStr = $form.find("input[name=couponEndTimeStr]").val();
        //var imageUrl = $form.find("input[name=file]").val();
        ...
        if(isEmpty(couponValue) ){
            Modal.alert({msg : "面值金额必填", title : "提示", btnok : "确定"});
            return false;
        }
        if(!isMoney(couponValue)){
            Modal.alert({msg : "面值金额不正确", title : "提示", btnok : "确定"});
            return false;
        }
        ...
        if(isEmpty(couponBeginTimeStr)){
            Modal.alert({msg : "使用开始日期必填", title : "提示", btnok : "确定"});
            return false;
        }
        if(isEmpty(couponEndTimeStr)){
            Modal.alert({msg : "使用截止日期必填", title : "提示", btnok : "确定"});
            return false;
        }
