...
jQuery(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        subGrid : false,
        datatype: "json",
        postData:{catName:"",parentId:$("#parentId").val()},
        height: 'auto',
        colNames:['分类名称','分类图片','分类级别','是否显示','序号','操作'],
        colModel:[
            {name:'catName',index:'typeName',width:100},
    ...
    function operFuc(cellvalue, options, rowObject) {
        ...
        return '<a title="编辑" href="javascript:void(0);" ' +
            'onclick="editCt(\'' + rowObject.catId + '\',\'' + rowObject.catName + '\',\'' + rowObject.status + '\',\'' + rowObject.imageUrl + '\',\'' + rowObject.seqno + '\')" > ' +
    ...
    // 返回
    $("#goods-cat-back").click(function(){
        location.href = base + "goodsCat/goodsCatIndex";
    });
    ...
    // 初始化上传文件输入框
    $('#goodsCatPicture').ace_file_input({
        no_file:'选择文件...',
        btn_choose:'选择',
        btn_change:'修改',
        droppable:false,
    ...
    $("#updateGoodsCat").click(function(){
        ...
        $("#updateGoodsCat").text("正在添加...");

        $("#update-form-goodsCat").ajaxSubmit({
            type:"post",
            dataType:"json",
...
// 显示修改商品二级分类窗口
function editCt(catId,catName,status,imageUrl,sqeno){
    $("#updateModal-type").modal("show");// 打开模态窗口
    $("#goodsCatIdVal").val(catId);
    ...
    $("#update-form-goodsCat").find("#seqno").val(sqeno);
}
...
function init(){
    ...
    // 初始化上传文件输入框
    $('#updateModal-type').find('#imageUrl').ace_file_input({
        no_file:'选择文件...',
        btn_choose:'选择',
        btn_change:'修改',
        droppable:false,
