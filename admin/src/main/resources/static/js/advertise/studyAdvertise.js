jQuery(function ($) {
    ...
    jQuery(grid_selector).jqGrid({
        url: base+"advertise/getStudyAdvertise",
        subGrid: false,
        datatype: "json",
        height: 'auto',
        ...
        colModel: [
            ...
            {name: 'adverId', index: 'adverId', width: 90, formatter: editStudy}
        ],
        jsonReader : {
            root:"list",
            page: "pageNum",
...
function editStudy(cellvalue, options, rowObject) {

    return '<a title="使用" style="margin-left: 10px;" href="javascript:void(0);" ' +
        'onclick="editStudyImg(\'' + rowObject.adverId + '\')" > '+'<i class="ace-icon fa fa-pencil-square-o bigger-200"> ';
}
function editStudyImg(adverId) {

    location.href = base+"advertise/updateStudyImg?adverId="+adverId;
}
// 初始化上传文件输入框
