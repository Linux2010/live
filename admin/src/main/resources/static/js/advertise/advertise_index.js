jQuery(function ($) {
    ...
    jQuery(grid_selector).jqGrid({
        url: base+"advertise/getAdvertiseIndex",
        subGrid: false,
        datatype: "json",
        height: 'auto',
        ...
        colModel: [
            ...
            {name: 'adverId', index: 'adverId', width: 90, formatter: editIndexUpdate}
        ],
        jsonReader : {
            root:"list",
            page: "pageNum",
    ...
    function add() {
        ...
        location.href = base+"advertise/addAdver";
    }

    //轮播图修改
   function editIndexUpdate(cellvalue, options, rowObject) {

       return '<a title="使用" style="margin-left: 10px;" href="javascript:void(0);" ' +
           'onclick="editAdvertise(\'' + rowObject.adverId + '\')" > '+'<i class="ace-icon fa fa-pencil-square-o bigger-200"> ';
    }
    function editAdvertise(adverId) {

        location.href = base+"advertise/updatePage?adverId="+adverId;
    }
...
/*function edit(adverId) {
    $("#updateModal-type").modal("show");
    var advertise = selectById(adverId);
    if (isEmpty(advertise)){
        Modal.alert("没有该广告！");
        return false;
    }
    $("#adver_id").val(adverId);
    $("#adver_name_update").val(advertise.adverName);
    $("#name_update").val(advertise.name);
    $("#adver_img_url_update").val(advertise.adverImg);
...
function red(a) {
    $(a).css("color","red");
    $(a).siblings("span").css("color",'');
    var objid = $(a)["0"].id;
    $("#detail_id").val(objid);
    $("#adver_add_name").val($(a).html());
}*/
