...
jQuery(function ($) {
    ...
    jQuery(grid_selector).jqGrid({
        url: base + "note/getUserPage?r="+Math.random(),
        subGrid: false,
        datatype: "json",
        height: 'auth',
        ...
        colModel: [
            ...
            {name:'',index:'', width:90, fixed:true, sortable:false, resize:false,
                formatter: function(cellvalue, options, rowObject) {
                    return '<a type="button" class="btn btn-white btn-sm btn-primary" onclick="sendMessage(\''+rowObject.userId+'\')">发送短信</a>';
                }
            }
        ],
        jsonReader: {
...
var sendMessageAll = function(){
    ...
    if(isNotEmpty(obj) && obj.length>0){
        if(confirm("确认要给本页的"+obj.length+"位会员发送短信？")){
            var userIds ="";
            for(var str in obj){
...
var back = function(){
    location.href = base + "note/record";
}
