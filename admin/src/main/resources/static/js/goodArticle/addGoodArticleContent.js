$(function(){
    ...
    $.ajax({
        ...
        data: {goodArticleId : $("#goodArticleId").val()},
        dataType: "json",
    ...
    $("#uploadBtn").click(function(){
        // 先验证非空项
        if(!check($("#goodArticleId").val(),ue.getContent())){
            return false;
        }else{
            // 设置上传按钮不可用
            ...
            $.ajax({
                type:"post",
                data:{goodArticleId:$("#goodArticleId").val(),content:ue.getContent()},
                dataType:"json",
                url:base+"goodArticle/updateContent",
                success: function(data) {
                    // 设置上传按钮可用
                    ...
                    if("success" == data.status){
                        ...
                        setTimeout(function () {
                            window.location.href=base+"goodArticle/";
                        },1800)
                    }else{
    ...
    $("#backBtn").click(function(){
        location.href=base+"goodArticle/";
    });
    function check(goodArticleId, content) {
        var result = false;
        if(isEmpty(goodArticleId)){
