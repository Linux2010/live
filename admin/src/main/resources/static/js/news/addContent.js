$(function(){
    ...
    $("#uploadBtn").click(function(){
        ...
        if(!check($("#informationId").val(),ue.getContent())){
            return false;
        }else{
            // 设置上传按钮不可用
            ...
            $.ajax({
                type:"post",
                data:{informationId:$("#informationId").val(),content:ue.getContent()},
                dataType:"json",
                url:base+"inform/updateContent",
                success: function(data) {
                    // 设置上传按钮可用
                    ...
                    if("success" == data.status){
                        ...
                        setTimeout(function () {
                            window.location.href=base+"inform/";
                        },1800)
                    }else{
    ...
    function check(informationId, content) {
        var result = false;
        if(isEmpty(informationId)){
            Modal.alert({msg: "咨询编号不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        if(isEmpty(content)){
            Modal.alert({msg: "咨询内容不能为空！",title: "提示", btnok: "确定"});
            return result;
        }
        result = true;
        return result;
