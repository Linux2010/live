jQuery(function ($) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        url: base + "recsetting/tealist",
        subGrid: false,
        datatype: "json",
        height: 325,
        colNames: ['id', '教头名称'],
        colModel: [
            {name: 'user.userId', index: 'user.userId', width: 200, editable: true, hidden: true, key: true},
            {name: 'user.userName', index: 'user.userName', width: 400},
        ],
        jsonReader: {
            root: "list",
            page: "pageNum",
...
function add() {
    ...
    $.ajax({
        ...
        data: {'objid': ids.toString(),"mark":1},
        dataType: "json",
        success: function (data) {
            if (data.status == "success") {
                ...
                setTimeout(function () {
                    window.location.href = base + "recsetting/";
                },1800)
