...
<div class="modal-content">
    ...
    <div class="modal-body" style="max-height:600px; overflow:scroll; ">
        <form id="form-data" class="form-horizontal">
            ...
            <#if profitShareRelation??>
                <input type="hidden" name="parentUserId" value="${profitShareRelation.userId}"/>
                <div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="loginName">一级分润用户名</label>
                    <div class="col-sm-9">
                        <input type="text" class="col-xs-10 col-sm-3"  value="${profitShareRelation.userName}" readonly>
                    </div>
                </div>
            </#if>
            <div class="form-group">
                <label class="col-sm-3 control-label no-padding-right" for="loginName">二级分润用户名</label>
                <div class="col-sm-9">
                    <select id="form-field-select-1" class="col-xs-10 col-sm-3" name="userId">
                        <option value=""></option>
...
<script type="text/javascript">
    ...
    $("#addBtn").click(function () {
        $.ajax({
            async: true,
            url: base + "profitshare/relation/addRelation",
            type: "POST",
            ...
            success: function (data) {
                if ("success" == data.status){
                    ...
                    location.href = base + 'profitshare/relation/pageskip?relationId='+"${profitShareRelation.relationId}"+'&level=2';
                }else{
