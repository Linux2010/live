...
public class LoginController {
    ...
    @RequestMapping("/third/{type}/login")
    public String qqLogin(@PathVariable("type") String type, HttpServletRequest request, HttpServletResponse response){
        response.setContentType("text/html;charset=utf-8");

        try {
            IThirdLoginBean bean  = (IThirdLoginBean) SpringUtil.getBean(type+"LoginBean");
            response.sendRedirect(bean.getWebUrl(request));
        } catch (IOException e) {
            e.printStackTrace();
            logger.error("",e);
        }
