$(function(){
    ...
    function showcourseList(pageNum,pageSize,allIscrollState){
        $.ajax({
            async: false,
            type: "POST",
            url: $("#basePath").val()+"wap/study/today_course_more",
            ...
            success: function (data) {
                ...
                for (var i =0; i<a.length; i++){
                    var introStr = a[i].courseIntro;
                    if(introStr.length > 48){
                        introStr = introStr.substring(0,47)+"...";
                    }
                    var courseTitleStr = a[i].courseTitle;
                    if(courseTitleStr.length > 18){
                        courseTitleStr = courseTitleStr.substring(0,17)+"...";
                    }
                    htmlVal  += '<a href='+$('#basePath').val()+'wap/course/courseDetail?courseType='+a[i].tyval+'&courseId='+a[i].courseId+'><div class="word">';
                    htmlVal += '<div class="line"><img class="img" src="'+a[i].courseImg+'">'
                        +'<div class="priceLine"><span class="s1">'+courseTitleStr+'</span>';
                        if($("#userIdentity").val() == 2){
                            htmlVal+='<span class="s2"></span></div>';
                        }else {
                            htmlVal+='<span class="s2">'+check_(a[i].costPrice)+'</span></div>';
                        }
                       htmlVal +='<div class="pLine">'+introStr
                        +'</div><div class="dataLine"><span class="s1">教头：'+a[i].tname
                        +'</span><span class="s2">'+new Date(a[i].courseBeginTime).pattern("yyyy-MM-dd HH:mm")+'</span></div></div>';
                    htmlVal += '</div></a>';
