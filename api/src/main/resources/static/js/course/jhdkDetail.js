        ...
        success: function (data) {
            if(data != null){
                ...
                if(data.userIdentity == 1 && data.orderFlag == 0 && data.costPrice > 0){
                    ...
                    if((data.userIdentity == 2 || data.costPrice == 0) && data.orderFlag == 0){
                        $("#joinNow").css("display","block");
                    }
                }
                // 为江湖大课详细地址赋值
                $("#addressSpan").text(data.courseAddressDetail);
                // 为用户是否购买了该课程变量赋值
                orderFlagVal = data.orderFlag;
                // 为用户身份变量赋值
                userIdentityVal = data.userIdentity;
                // 为课程价格变量赋值
                costPriceVal = data.costPrice;
                // 展示江湖大课地图定位
                var mapArr = [117.000923,36.675807];
                if(data.courseAddress != null && data.courseAddress != ""){
                    mapArr[0] = data.courseAddress.split(",")[1];
                    mapArr[1] = data.courseAddress.split(",")[0];
                    $("#xVal").val(mapArr[0]);
                    $("#yVal").val(mapArr[1]);
                }
...
function joinCourse(){
    if(orderFlagVal == 0){
        ...
        if(userIdentityVal == 2 || costPriceVal == 0){
            // 初始化课程分类
            $.ajax({
                async: false,
                type: "POST",
...
// 返回江湖大课列表
function backFuc(){
    location.href = $("#basePath").val()+"wap/course/jhdkIndex";
}

// 展示高德地图
function showGdMap(){
    location.href = $("#basePath").val()+"wap/course/showGdMap?xVal="+$("#xVal").val()+"&yVal="+$("#yVal").val();
}