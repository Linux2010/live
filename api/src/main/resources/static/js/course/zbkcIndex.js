$(function(){
    ...
    $("#searchSpan").click(function(){
        ...
        showZbkcList(1,10,$("#courseTitle").val(),1);
    });
    ...
    function pullUpAction(){
        setTimeout(function(){
            curpage++;
            showZbkcList(curpage,10,$("#courseTitle").val(),0);
        }, 1000)
    }
    ...
    // 展示直播课程列表
    function showZbkcList(pageNum,pageSize,courseTitle,allIscrollState){
        $.ajax({
            async: true,
            type: "POST",
            url: $("#basePath").val() + "wap/course/showZbkcList",
                    ...
                    for(var i = 0;i < data.length;i++){
                        ...
                        if(data[i].userIdentity == 2) {// 如果当前登录用户是会员，则隐藏价格显示
                            htmlVal += "<div class='line' onclick='showCourseDetail(\""+data[i].courseId+"\")'><img class='img' src='"+data[i].courseImg+"'>"
                                +"<div class='priceLine'><span class='s1'>"+courseTitleStr+"</span></div></div>";
                        }else{
                            htmlVal += "<div class='line' onclick='showCourseDetail(\""+data[i].courseId+"\")'><img class='img' src='"+data[i].courseImg+"'>"
                                +"<div class='priceLine'><span class='s1'>"+courseTitleStr+"</span><span class='s2'>"+costPriceVal+"</span></div></div>";
                        }
                    }
                    htmlVal += "</div>";
...
function showCourseDetail(couserId){
    location.href = $("#basePath").val()+"wap/course/courseDetail?courseType=zbkc&courseId="+couserId;
}
