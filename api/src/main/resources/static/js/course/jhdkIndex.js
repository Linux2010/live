$(function(){
    ...
    $("#searchSpan").click(function(){
        ...
        showJhdkList(1,10,$("#courseTitle").val(),1);
    });
    ...
    function pullUpAction(){
        setTimeout(function(){
            curpage++;
            showJhdkList(curpage,10,$("#courseTitle").val(),0);
        }, 1000)
    }
    ...
    // 展示江湖大课列表
    function showJhdkList(pageNum,pageSize,courseTitle,allIscrollState){
        $.ajax({
            async: true,
            type: "POST",
            url: $("#basePath").val() + "wap/course/showJhdkList",
                    ...
                    for(var i = 0;i < data.length;i++){
                        ...
                        if(data[i].userIdentity == 2) {// 如果当前登录用户是会员，则隐藏价格显示
                            htmlVal += "<div class='line' onclick='showCourseDetail(\""+data[i].courseId+"\")'><img class='img' src='"+data[i].courseImg+"'>"
                                ...
                                +"</div><div class='dataLine'><span class='s1'>教头："+data[i].tname
...
function showCourseDetail(couserId){
    location.href = $("#basePath").val()+"wap/course/courseDetail?courseType=jhdk&courseId="+couserId;
}
