    ...
    $.ajax({
        ...
        success: function (data) {
            if(data != null){
                ...
                $("#teaId").val(data.teacherId);
                $("#imRoomid").val(data.imRoomid);
    ...
    if(new Date().getTime() < $("#courseBeginTime").val()){
        ...
        if(orderFlagVal == 0){
            ...
            if(userIdentityVal == 2 || costPriceVal == 0){
                if(userIdentityVal == 0){// 如果未登录，该直播课程价格为0，则可以观看直播
                    location.href = $("#basePath").val()+"wap/course/showRoom?teaId="+$("#teaId").val()+"&imRoomid="+$("#imRoomid").val();
                }else{
                    // 初始化课程分类
                    $.ajax({
                        async: false,
...
// 返回直播课程列表
function backFuc(){
    location.href = $("#basePath").val()+"wap/course/zbkcIndex";
}