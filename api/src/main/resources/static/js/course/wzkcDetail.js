    ...
    $.ajax({
        ...
        success: function (data) {
            if(data != null){
                ...
                $("#courseBeginTime").val(data.courseBeginTime);
                ...
                if(data.orderFlag != 0){
                    if(data.ktYnFlag == 1){// 如果有答题，则提示是否开始答题
                        if(data.dtFlag == 0){// 如果未答过题，则提示是否开始答题
                            if(data.version == 2){// 代表着已观看了文章
                                setTimeout(showDt, 2000);// 延时2秒执行
                            }
                        }
                    }
                }
...
// 展示文字课程内容
function showWzkcContent(){
    if(new Date().getTime() < $("#courseBeginTime").val()){
        alert("课程还未开始");
        return false;
    }else{
        if(orderFlagVal == 0){
            ...
            $.ajax({
                async: false,
                type: "POST",
                url: $("#basePath").val()+"wap/course/modifyWzkcFlag",
                data: {courseId:$("#courseId").val()},
                dataType: "json",
                success: function (data) {
                    if(data != null && data == 0){
                        if($("#courseContent").val() == null || $("#courseContent").val() == ""){
                            alert("该课程尚未上传文章");
                            return false;
                        }else{
                            location.href = $("#courseContent").val();
                        }
                    }
                },
                error:function(data){
...
// 返回文字课程列表
function backFuc(){
    location.href = $("#basePath").val()+"wap/course/wzkcIndex";
}