$(function(){
    ...
    function showWzkcList(pageNum,pageSize,allIscrollState){
        $.ajax({
            async: true,
            type: "POST",
            url: $("#basePath").val() + "wap/myteam/myCollectionList",
            data: {pageNum:pageNum,pageSize:pageSize},
            dataType: "json",
            success: function (data) {
                var htmlVal = "<div class='con3'>";
                if(data != null && data.length > 0){
                    for(var i = 0;i < data.length;i++){
                        htmlVal+= '<div class="con3Item">'
                            +'<a href="'+$("#basePath").val()+'wap/course/courseDetail?courseType='+data[i].tyval+'&courseId='+data[i].courseId+'"><img class="con3ItemLeft" src="'+data[i].courseImg+'">'
                            +'<div class="con3ItemRight">'
                            +'<span class="con3ItemTitle" style="display: inline-block;width: 3rem;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">'+data[i].courseTitle+'</span>'
                            +'<div class="con3ItemBody">'
                            +'<div class="con3ItemTime">'
                            +'<p>讲师：'+data[i].tname+'</p>'
                            +'<p>'+new Date(data[i].courseBeginTime).pattern("yyyy-MM-dd")+'</p>'
                            +'</div>'
