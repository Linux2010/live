 ...
 * 聊天室逻辑
 */
var wrapperWordH = window.innerHeight-(8.5*100 * (window.innerWidth / 750));
$(".tabBodyIscroll").height(wrapperWordH+"px");
$(".tab .tabNav .navCon").click(function () {
    $(this).addClass("active").siblings().removeClass("active");
    $(".tabBody").eq($(this).index()).addClass("activity").siblings().removeClass("activity");
    if($(this).index()!==0){
        $(".sendCon").hide();
    }else{
        $(".sendCon").show();
...
var wrapperWord = new IScroll("#wrapperWord",{
    scrollX: false,
    scrollY: true,
    hScrollbar:true,
    vScrollbar : false,
...
var page = {
	init:function () {
        var options = {
            ...
            account: $("#accId").val(),// 当前登录用户的accid
            id: $("#imRoomid").val(),// 直播间Id
            token: $("#accidToken").val(),// 当前登录用户的accidtoken
            chatroomNick:$("#userName").val(),// 当前登录用户的用户名
            chatroomAvatar:util.readCookie("avatar")
        }
        // 直播间房主的accid
        this.account = $("#teaAccid").val();
        this.offsetTime = 0;
    	this.initNode();
		this.initEvt();
	...
	initNode:function(){
        ...
        this.$zhuzi =$("#zhuzi");
        //礼物数量
        this.$zhuziNum = parseInt($("#presentNum").val());
        //礼物类型
		this.$presentType = parseInt($("#presentType").attr("name"));
		// this.$chat = $("#room .j-chat");
		this.$chat = $("#wrapperWord .scroller .room");
	...
	initVideo:function(){
        $.ajax({
            async: false,
            type: "POST",
            url: $("#basePath").val()+"wap/course/searchRoomInfo",
            data: {imRoomId:$("#imRoomid").val()},
            dataType: "json",
            success: function (data) {
                if(data != null){
                    if(data.name == "0"){
                        ...
                        // 给在线人数赋值
                        $("#olSpan").text(data.onlineUserCount);
                        // 给播放器拉流，进行直播
                        var myPlayer = neplayer("my-video");
                        myPlayer.setDataSource([
                            {src:data.httppullurl,type:"video/x-flv"},
                            {src:data.hlspullurl,type:"application/x-mpegURL"},
                            {src:data.pullUrl,type:"rtmp/flv"}
                        ]);
                        // 直接播放
                        myPlayer.play();
                    }
	...
	showLaowang:function(){
		...
		if($("#accId").val() == null || $("#accId").val() == ""){// 如果未登录，则不能发送消息
    ...
    showZhuzi:function(){
		// 验证礼物数量的输入
		if($("#presentNum").val() == null || $("#presentNum").val() == ""){
			alert("请输入礼物的数量");
			return false;
		}else{
            if(isNaN($("#presentNum").val())){
                alert("请以数字的形式输入礼物数量");
                return false;
            }else{
            	if(parseInt($("#presentNum").val()) == 0){
                    alert("请输入大于零的数字");
                    return false;
				}else{
            		if(parseInt($("#presentNum").val()) > 100){
                        alert("一次最多可送100个礼物");
                        return false;
					}
				}
        ...
        $.ajax({
            async: false,
            type: "POST",
            url: $("#basePath").val()+"wap/course/sendGift",
            data: {
            	imRoomId:$("#imRoomid").val(),
				giftId:$("#presentType").attr("name"),
				giftNum:$("#presentNum").val()
			},
            dataType: "json",
            success: function (data) {
                if(data != null){
        ...
        if(flagVal){
            var content ={
                type:0,
                data:{
                    present:parseInt($("#presentType").attr("name")),
                    count:parseInt($("#presentNum").val()),
                },
            };
            this.link.sendCustomMessage(content,function(err,data){
                if(err){
	...
	buildChat:function(data,type){
		...
		if(type==="msgs"){
            ...
            var charCon = $("#wrapperWord .scroller .room");
            $("#wrapperWord .scroller").height(charCon.height()+"px");
            if(charCon.height()>wrapperWordH){
                wrapperWord.scrollTo(0,-(charCon.height()-wrapperWordH));
                wrapperWord.refresh();
            }
		}else{
			// 历史消息
	...
	buildMsgUI:function(item){
		...
		if(item.time != null && item.time != ""){
            ...
            timeVal = time.getHours()+":"+time.getMinutes()+":"+time.getSeconds();
		}
		var msgVal = "进入直播间";
		if(util.buildMsg(item) != null && util.buildMsg(item) != ""){
            msgVal = util.buildMsg(item);
		}
		return ['<div class="item" data-id="'+item.idClient+'">',
				...
				'<div class="wordCon">',
           		'<div class="wordTimeLine"><span>'+item.fromNick+'</span>&nbsp;&nbsp;<span>'+timeVal+'</span></div>',
				'<div class="wordLine"><span class="word">'+msgVal+'</span></div>',
                '</div>',
				'</div>'].join("");
        // return ['<div class="item" data-id="'+item.idClient+'">','</div>'].join("");
	},
	//系统消息UI绘制
	buildSysMsgUI:function(item){
		return ['<div class="item" data-id="'+item.idClient+'">',
            ...
            '<div class="wordCon">',
            '<div class="wordLine"><span class="word sys">'+util.buildSysMsg(item)+'</span></div>',
            '</div>',
				'</div>'].join("");
	},
	//成员列表UI绘制
