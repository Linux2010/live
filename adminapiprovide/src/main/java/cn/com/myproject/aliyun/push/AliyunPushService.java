...
public class AliyunPushService implements IAliyunPushService {
    ...
    private final String iOSApnsEnv = "PRODUCT";//iOS的通知是通过APNs中心来发送的，需要填写对应的环境信息。"DEV" : 表示开发环境 "PRODUCT" : 表示生产环境

    //private final String iOSApnsEnv = "PRODUCT;
...
//        pushRequest.setTargetValue("ALL"); //根据Target来设定，如Target=device, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
        ...
        pushRequest.setiOSApnsEnv(iOSApnsEnv);//iOS的通知是通过APNs中心来发送的，需要填写对应的环境信息。"DEV" : 表示开发环境 "PRODUCT" : 表示生产环境
    ...
    @Override
    public Map<String, String> pushNotice(String title, String body, String account, String extParameters) {
        Map<String,String> resultMap = new HashMap<>();
        String status = "fail";
        String message = "调用失败";
        IClientProfile profile = DefaultProfile.getProfile("cn-hangzhou", aliyunPushProp.getAppKeyId(), aliyunPushProp.getAppSecret());
